...
2. **AI Analysis Layer (`ai_insights.py`)**: Responsible for constructing the context window for the AI. It relies heavily on **OpenAI Structured Outputs** via integer and string-backed Pydantic models. It sends the factual metrics alongside the text content and ensures a predictable JSON schema is returned containing actionable insights.
...
2. **Strict Output Formatting Guidelines**: We enforce a predictable output structure by injecting a rigid `OUTPUT STRUCTURE` template into the system prompt. By heavily defining the expected markdown headers (e.g., `1. SEO STRUCTURE`, `6. PRIORITIZED RECOMMENDATIONS`, `7. OVERALL SCORE (0–100)`), we can parse out the exact scores programmatically via regex in the frontend while still allowing the LLM to write flowing contextual paragraphs.
...
* **Heuristic CTA Detection**: Rather than spinning up a full headless browser to analyze CSS classes and visual placement, we are detecting `<button>`, `<input type="submit">`, and `<a>` elements containing common action text ("buy", "sign up"). This is lighter and faster, but misses visual buttons that don't stick to these heuristics.
...
* **Single-Page Constraint**: Metrics like "Internal Links" are pulled, but we don't follow them to gauge site-wide structure. The tool provides a micro-audit, not a macro-audit.
...
4. **Usage:**
   ...
   - Check `prompt_logs/prompt_logs.json` for the full prompt trace
...
the exact inputs sent to the model and the raw output before formatting.